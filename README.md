...
command will obtain an initial certificate and configure a Cron job to
periodically renew it and reload Vault.

It assumes the existence of an `hdb-credentials` file on disk. The credentials
can be found in the Vault itself under `secret/certbot-hdb/credentials`,
assuming the Vault is up and running in the first place. If the Vault contents
and these credentials are lost, we would need to obtain new credentials from
ICT. See the [lego-hdb][lego-hdb] for more details about the file's expected
format.
...
[lego-hdb]: https://github.com/reside-ic/lego-hdb
